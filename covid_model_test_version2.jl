...
for region in regions
  ...
  elseif region == "EN"
    # set data
    data = readdlm("all_data_to_export_version2.csv",',')[:,29:32]
    # data = readdlm("datafit_EN.csv",',')[:,1:3];
    global hospital_incidence_data  = [data[i,1] for i in 1:length(data[:,1]) if isnan.(data[i,1])==0][1:end-2]; #data[:,1];
    global hospital_prevalence_data = [data[i,2] for i in 1:length(data[:,2]) if isnan.(data[i,2])==0];# data[:,2];
    global icu_prevalence_data      = [data[i,3] for i in 1:length(data[:,3]) if isnan.(data[i,3])==0];# data[:,3];
  ...
  # Initial conditions
  final_prediction_day = 204;
  prediction_length = 220;
  ...
  posterior_samples = random_walk(delayed_ode,
                                  ...
                                  covariance_matrix,
                                  1)
  writedlm(string("$region","_output_test_version2.csv"),posterior_samples,',')

  # do this once you have a covariance matrix for decent posteriors
  output = readdlm(string("$region","_output_test_version2.csv"),',');
  ...
  savefig(plot(chain),string("$region","_chain_test_version2.png"))

  # make predictions
  posterior_samples = readdlm(string("$region","_output_test_version2.csv"),',');
  ...
  writedlm(string("$region","_hi_nbinom_output_test_version2.csv"),transpose(prediction[1,10:number_of_days-5,:]),',')
  writedlm(string("$region","_hp_nbinom_output_test_version2.csv"),transpose(prediction[2,10:number_of_days-5,:]),',')
  writedlm(string("$region","_up_nbinom_output_test_version2.csv"),transpose(prediction[3,10:number_of_days-5,:]),',')
  writedlm(string("$region","_di_nbinom_output_test_version2.csv"),transpose(prediction[4,10:number_of_days-5,:]),',')

  # compute growth rate
  hi_growth_rate = transpose((log.(vec(rIH*predicted_states[:,6,final_prediction_day]./(posterior_samples[:,6].-1))) .-
                              log.(vec(rIH*predicted_states[:,6,final_prediction_day-7]./(posterior_samples[:,6].-1)))) ./ 7)
  hi_growth_rate = replace(hi_growth_rate, NaN => -Inf);
  writedlm(string("$region","_growth_rate_hi_test_version2.csv"),transpose(quantile(vec(hi_growth_rate),0.05:0.05:0.95)),',');

  # compute r values
  all_r_values = posterior_samples[:,4] .* (predicted_states[:,1,final_prediction_day] ./ initial_population);
  writedlm(string("$region","_R_value_adjusted_version2.csv"),transpose(quantile(all_r_values,0.05:0.05:0.95)*3.62),',');
  ...
  di_quantiles = [quantile(prediction[4,i,:],j) for i in final_prediction_day-20:final_prediction_day,
                                                    j in 0.05:0.05:0.95];
  writedlm(string("$region","_posterior_predicted_quantiles_test_version2.csv"),[hi_quantiles;
                                                                       up_quantiles;
                                                                       di_quantiles],',');
  ...
  savefig(string("$region","_posterior_hospital_incidence_test_version2.pdf"))
  ...
  savefig(string("$region","_posterior_hospital_prevalence_test_version2.pdf"))
  ...
  savefig(string("$region","_posterior_icu_prevalence_test_version2.pdf"))
  ...
  savefig(string("$region","_posterior_death_test_version2.pdf"))
end
