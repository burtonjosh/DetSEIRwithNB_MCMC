...
function log_likelihood(position::Array{Float64})
  ...
  # set support constraints for parameters
...
global regions = ["EN"]; #["EN","EE", "LO", "MI", "NE", "NW", "SE", "SW", "UK"];
for region in regions
  ...
  # Initial conditions
  final_prediction_day = 204; # increase by seven each week
  prediction_length = 220;# increase by seven each week
  ...
  #                                 covariance_matrix,
  #                                 1)
  # writedlm(string("$region","_output_probs.csv"),posterior_samples,',')

  # do this once you have a covariance matrix for decent posteriors
  output = readdlm(string("$region","_output_probs.csv"),',');
  ...
  chain = Chains(posterior_samples,["β₁","β₂","β₃","β₄","log(J₀)","σ_hi","σ_hp",
                                    "σ_up","σ_di","pC","pT","pD"]);
  savefig(plot(chain),string("$region","_chain_probs_test.png"))

  # make predictions
  posterior_samples = readdlm(string("$region","_output_probs.csv"),',');
  ...
  writedlm(string("$region","_hi_nbinom_output_probs.csv"),transpose(prediction[1,10:number_of_days-5,:]),',')
  writedlm(string("$region","_hp_nbinom_output_probs.csv"),transpose(prediction[2,10:number_of_days-5,:]),',')
  writedlm(string("$region","_up_nbinom_output_probs.csv"),transpose(prediction[3,10:number_of_days-5,:]),',')
  writedlm(string("$region","_di_nbinom_output_probs.csv"),transpose(prediction[4,10:number_of_days-5,:]),',')
  ...
  writedlm(string("$region","_growth_rate_hp_probs.csv"),transpose(quantile(vec(hp_growth_rate),0.05:0.05:0.95)),',');
  ...
  di_quantiles = [quantile(prediction[4,i,:],j) for i in final_prediction_day-20:final_prediction_day,
                                                    j in 0.05:0.05:0.95];
  writedlm(string("$region","_posterior_predicted_quantiles_probs.csv"),[hi_quantiles;
                                                                       hp_quantiles;
                                                                       up_quantiles;
                                                                       di_quantiles],',');
  ...
  savefig(string("$region","_posterior_hospital_incidence_probs.pdf"))
  ...
  savefig(string("$region","_posterior_hospital_prevalence_probs.pdf"))
  ...
  savefig(string("$region","_posterior_icu_prevalence_probs.pdf"))
  ...
  savefig(string("$region","_posterior_death_probs.pdf"))
