...
for region in regions
  ...
  #                                 covariance_matrix,
  #                                 1)
  # writedlm(string("$region","_output_probs_version2.csv"),posterior_samples,',')

  # do this once you have a covariance matrix for decent posteriors
  output = readdlm(string("$region","_output_probs_version2.csv"),',');
  ...
  chain = Chains(posterior_samples,["β₁","β₂","β₃","β₄","log(J₀)","σ_hi","σ_hp",
                                    "σ_up","σ_di","pC","pT","pD"]);
  savefig(plot(chain),string("$region","_chain_probs_version2.png"))

  # make predictions
  posterior_samples = readdlm(string("$region","_output_probs_version2.csv"),',');
  ...
  writedlm(string("$region","_hi_nbinom_output_probs_version2.csv"),transpose(prediction[1,10:number_of_days-5,:]),',')
  writedlm(string("$region","_hp_nbinom_output_probs_version2.csv"),transpose(prediction[2,10:number_of_days-5,:]),',')
  writedlm(string("$region","_up_nbinom_output_probs_version2.csv"),transpose(prediction[3,10:number_of_days-5,:]),',')
  writedlm(string("$region","_di_nbinom_output_probs_version2.csv"),transpose(prediction[4,10:number_of_days-5,:]),',')
  ...
  writedlm(string("$region","_growth_rate_hi_probs_version2.csv"),transpose(quantile(vec(hi_growth_rate),0.05:0.05:0.95)),',');
  ...
  di_quantiles = [quantile(prediction[4,i,:],j) for i in final_prediction_day-20:final_prediction_day,
                                                    j in 0.05:0.05:0.95];
  writedlm(string("$region","_posterior_predicted_quantiles_probs_version2.csv"),[hi_quantiles;
                                                                       up_quantiles;
                                                                       di_quantiles],',');
